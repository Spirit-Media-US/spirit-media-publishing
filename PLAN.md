...
| API tokens (Sanity, Netlify, Cloudflare) | VPS env vars in `/etc/environment` or `~/.profile` | Loaded by shell, available to all processes |
...
| Biome (linter/formatter) | Global install: `npm i -g @biomejs/biome` | Same lint rules everywhere |
| Lefthook (git hooks) | Global install: `npm i -g lefthook` v2 | Pre-commit hooks run on every repo |
| Lefthook config | `/home/deploy/.config/lefthook/lefthook.yml` (global) | Shared hook definitions |
...
| biome.json | `<repo>/biome.json` (committed) | Can override global rules if needed |
| lefthook.yml | `<repo>/lefthook.yml` (committed) | Can extend global hooks if needed |

**Key rule:** If a config applies to ALL sites, it goes at the VPS system level. If it's unique to ONE site, it goes in the project. CLAUDE.md files should ONLY contain project-specific information.
...
| 5 | Global lefthook config | Create `/home/deploy/.config/lefthook/lefthook.yml` |
...
3. Set personal `ANTHROPIC_API_KEY` in `~/.bashrc` (or use Claude Max)
...
- `biome --version` on VPS → installed globally
- `lefthook version` on VPS → v2 installed globally
